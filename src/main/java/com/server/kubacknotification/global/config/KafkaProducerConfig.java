...
public class KafkaProducerConfig {
	...
	public ProducerFactory<String, PaymentMessage> paymentMessageProducerFactory() {
		...
		configProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class); // 메시지를 JSON으로 직렬화
	...
	@Bean
	public ProducerFactory<String, TicketOpenMessage> ticketOpenMessageProducerFactory() {
		Map<String, Object> configProps = new HashMap<>();
		configProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
		configProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
		configProps.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class); // 메시지를 JSON으로 직렬화
	...
	@Bean
	public KafkaTemplate<String, TicketOpenMessage> ticketOpenkafkaTemplate() {
		return new KafkaTemplate<>(ticketOpenMessageProducerFactory());
	}
}
